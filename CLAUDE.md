...
- **Learn from corrections** — Weavie uniquely sees the user's *net edit over the agent's output*
  ...
  and a palette-only `/learn` command feeds the corpus to the primary session's Claude to propose
  `CLAUDE.md` rules — Weavie stores the signal, Claude does the reasoning (no classifier in Core). A
  contextual-suggestion card nudges once enough accumulate. Spec'd, not yet built. See
