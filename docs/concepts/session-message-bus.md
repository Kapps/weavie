...
`lifecycle.sync` snapshot after hello, not by replaying arbitrary commands. Agent history uses an authenticated,
exact-session HTTP response stream, independent of live message traffic. Sync snapshots are unicast to the requesting
