...
2. **Control status line** — a dim strip under the composer showing the session's **model**, reasoning
	**effort**, **speed** (standard or Fast), **approvals**, and **sandbox**. Each segment opens a picker; a
	change applies **live to the running session**.
...
accepts `model`, `effort`, `serviceTier`, `approvalPolicy`, and `sandboxPolicy`. So a live
