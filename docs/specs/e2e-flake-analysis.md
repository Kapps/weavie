...
## CONFIRMED + FIXED: Linux `LinuxWaylandIdentityTests` app-boot wait was too tight, not stuck

**Symptom, 2026-08-21 04:35 and 04:41 UTC, runs 32447336985 and 32447666278:**
[job 96669255485](https://github.com/Kapps/weavie/actions/runs/32447336985/job/96669255485) and
[job 96670244185](https://github.com/Kapps/weavie/actions/runs/32447666278/job/96670244185), PR #615
(diff touched only `src/web/index.html` and this doc — unrelated) — `.NET tests` /
`Weavie.Linux.Tests.LinuxWaylandIdentityTests.LinuxHost_PublishesTheDesktopAppIdToWayland` failed twice
in a row, back-to-back CI runs on the same PR, with `System.TimeoutException` at the app-boot wait
(`tests/Weavie.Linux.Tests/LinuxWaylandIdentityTests.cs`,
...
`ace5300`) runs immediately before this wait and evidently passed cleanly both times — the compositor
handshake is not what timed out. This time the spawned app itself (`dbus-run-session` launching the full
...
**Why widening the budget here is a real fix, not a safety net:** the wait is already gated on a real,
specific signal — `appIdPublished` only completes when the app's own stdout emits its `set_app_id(...)`
line, not a blind sleep. Two back-to-back misses on the exact same 15s budget, with no other evidence of
a genuine hang (the compositor gate right before it passed cleanly both times), is a measured signal that
a cold GTK4/WebKitGTK boot under current CI load needs more than 15s, not that the app is stuck. Widened
to 30s, mirroring the same calibration already applied to the compositor wait two lines above it in the
same test. Not verified locally (this sandbox lacks `libdrm-dev`/`weston`, so this test can't build here)
— validate by watching for a further recurrence on subsequent Linux CI runs.
