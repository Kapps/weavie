...
/// <c>~/.weavie/keybindings.json</c>. Shipped to the web (which resolves keydowns against it) and surfaced in
/// <c>listCommands</c>. <see cref="Key"/> keeps the tinykeys-style <c>$mod</c> form; the web formats it.
/// </summary>
public sealed record ResolvedKeybinding {
	...
	/// <summary>
	/// When true, the host registers this as an OS-level global hotkey (fires even when Weavie is unfocused)
	/// and the web keydown resolver skips it. See <see cref="CommandKeybinding.Global"/>.
	/// </summary>
