...
internal sealed partial class LinuxWorkspaceDirectoryWatchSet {
	private void ReadAvailableEvents(byte[] buffer) {
		...
		long deadline = Environment.TickCount64 + MovePairingWindowMilliseconds;
		while (movedFrom.Count > 0) {
			int remaining = (int)Math.Max(0, deadline - Environment.TickCount64);
			if (remaining == 0 || !WaitForMovePairEvents(remaining)) {
				break;
			}
	...
	// inotify may expose a move pair across separate reads; unmatched cookies are moves out of the watched tree.
	private unsafe bool WaitForMovePairEvents(int timeoutMs) {
		...
		while (true) {
			...
			int result = poll(pollFds, 2, timeoutMs);
			if (result < 0) {
				if (Marshal.GetLastPInvokeError() == Interrupted) {
					continue;
				}

				throw NativeFailure("poll(move pair)");
			}

			if (result == 0 || (pollFds[1].ReturnedEvents & PollIn) != 0) {
				return false;
			}
			...
			return (pollFds[0].ReturnedEvents & PollIn) != 0;
		}
	}
