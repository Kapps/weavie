...
/// <summary>
/// The relay side of the hook bridge: the program Claude runs as its <c>command</c> hook. Forwards the hook
/// JSON from stdin to the in-process <c>HookBridgeServer</c> over the <see cref="HookProtocol.PipeEnvVar"/>
/// pipe and writes the decision back to stdout. Fails open — any error exits 0 with no stdout, so a recorder
/// hiccup never blocks a tool. Lives in the standalone <c>Weavie.HookRelay</c> executable.
/// </summary>
