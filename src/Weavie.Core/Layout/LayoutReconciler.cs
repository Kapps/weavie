...
/// newly-introduced defaults, bump the seen-level watermark, normalize and repair — are the entire
...
public static class LayoutReconciler {
	...
	public static ReconcileOutcome Reconcile(LayoutDocument document, PaneRegistry registry) {
		...
		// 1. Prune panes whose kind is no longer registered; collapse and renormalize what remains.
		var root = Prune(document.Root, registry, notes) ?? FallbackRoot(registry, notes);
