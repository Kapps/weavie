...
public sealed partial class McpServer {
	...
	private const string SettingsToolEntries =
		"""
          ...
          {"name":"clearSetting","description":"Reset a weavie setting to its default by removing its user-file override (the inverse of setSetting). Call getSetting first to confirm the key and that its source is 'userFile'. No-op if there is no user override. If an environment variable also overrides the setting, the effective value won't change until that var is unset.","inputSchema":{"type":"object","properties":{"key":{"type":"string"}},"required":["key"]}}
        """;
