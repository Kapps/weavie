...
/// (rather than each shell hand-rolling — and drifting on — its own leaf-name logic).
/// </summary>
public static class WorkspaceNaming {
	/// <summary>
	/// The folder's leaf name for the window title / shell label (e.g. <c>weavie</c> for <c>/src/weavie</c> or
	/// <c>C:\src\weavie</c>), trimming either separator; falls back to the full root when there's no leaf (a drive root).
	/// </summary>
