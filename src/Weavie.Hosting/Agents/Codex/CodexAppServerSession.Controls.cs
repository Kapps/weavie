...
public sealed partial class CodexAppServerSession : IStructuredAgentControls {
	...
	private static readonly IReadOnlyList<AgentControlOption> EffortOptions = [
		new() { Id = "low", Label = "Low", Description = "Faster responses with lighter reasoning." },
		new() { Id = "medium", Label = "Medium", Description = "Balance speed and reasoning depth." },
		new() { Id = "high", Label = "High", Description = "More reasoning for complex work." },
		new() { Id = "xhigh", Label = "Extra high", Description = "Maximum reasoning depth for difficult work." },
	];

	private static readonly IReadOnlyList<AgentControlOption> SpeedOptions = [
		new() { Id = "standard", Label = "Standard", Description = "Use the normal Codex service tier." },
		new() { Id = "priority", Label = "Fast", Description = "Faster processing with increased usage." },
	];
	...
	private string _effortOverride = "medium";
	private string _serviceTierOverride = "standard";
	...
	private string EffectiveServiceTier() {
		lock (_gate) {
			return _serviceTierOverride == "priority" ? "priority" : string.Empty;
		}
	}
	...
	private AgentControlState BuildControlState() {
		string model, sandbox, approval, effort, serviceTier, catalogDefault;
		...
		lock (_gate) {
			...
			effort = _effortOverride;
			serviceTier = _serviceTierOverride;
		...
		List<AgentControlAxis> axes = [
			...
			Axis("effort", "Effort", effort, EffortOptions),
			Axis("serviceTier", "Speed", serviceTier, SpeedOptions),
	...
	private bool IsValidControl(string axis, string value) {
		switch (axis) {
			...
			case "effort":
				return EffortOptions.Any(option => option.Id == value);
			case "serviceTier":
				return SpeedOptions.Any(option => option.Id == value);
			case "sandbox":
