...
public sealed partial class CodexAppServerSession : IStructuredAgentSession {
		...
		try {
			return await _client.RequestAsync(
				...
				CodexAppServerProtocol.ThreadResume(
					requestId, threadId, Model(), _context.Workspace, Sandbox(), ApprovalPolicy()),
	...
	private Task<JsonElement> StartThreadAsync(long requestId) =>
		_client.RequestAsync(
			requestId,
			CodexAppServerProtocol.ThreadStart(requestId, Model(), _context.Workspace, Sandbox(), ApprovalPolicy()),
