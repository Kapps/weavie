...
public static class InferenceComposition {
	/// <summary>Creates the built-in operation catalog over the installed agent providers and live settings.</summary>
	public static IInferenceService CreateDefault(SettingsStore settings, AgentProviderRegistry agentProviders) {
		...
		return new InferenceService(settings, CoreInferenceOperations.CreateRegistry(), agentProviders);
	}
	...
	public static IInferenceService CreateDisabled(SettingsStore settings) {
		...
		return new InferenceService(
			settings,
			CoreInferenceOperations.CreateRegistry(),
