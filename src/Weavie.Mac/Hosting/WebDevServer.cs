...
/// already serving the port, or otherwise spawns Vite itself (so there's no second terminal to
/// babysit), waits for it to start serving, and the caller points the WKWebView at
...
/// server is unavailable, so the caller can fall back to the bundled <c>app://</c> wwwroot.
/// </summary>
internal sealed class WebDevServer : IDisposable {
	...
	// 127.0.0.1) so the origin matches the canonical URL Vite serves and prints.
	...
	/// Debug build injects (see Weavie.Mac.csproj), normalized to a full path that exists.</summary>
