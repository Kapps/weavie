...
/// <summary>
/// Toggles WebKit feature flags on a <see cref="WKPreferences"/> via private SPI — the same surface
/// Safari's "Feature Flags" pane exposes. Used to turn off "Prefer Page Rendering Updates near 60fps"
/// so WKWebView renders at the display's full refresh (120Hz ProMotion) instead of being paced to 60.
///
/// The feature list comes from the <b>class</b> property <c>+[WKPreferences _features]</c> (an array
/// of <c>_WKFeature</c>), and the per-instance setter is <c>-[WKPreferences _setEnabled:forFeature:]</c>.
///
/// PRIVATE API: fine for this local tool, NOT App Store safe. Guarded by <c>respondsToSelector:</c>
/// and logs what it did (incl. candidate keys when the exact flag is missing), so a future WebKit
/// that drops/renames the SPI just no-ops back to 60Hz instead of crashing.
/// </summary>
internal static partial class WebKitFeatureFlags {
	...
	/// <summary>
	/// Turns off the "prefer 60fps" rendering pace so the page can update at the display's full
	/// refresh rate. No-ops (with a log line) if the SPI or flag is unavailable.
	/// </summary>
