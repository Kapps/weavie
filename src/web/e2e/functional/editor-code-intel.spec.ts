...
// leaving only Cut / Copy / Paste / Command Palette. This change restores three first-class Weavie commands
// ABOVE the clipboard group — Go to Definition (F12), Find All References (Shift+F12), Rename Symbol (F2) —
// each advertising its shortcut (read live from the command catalog via formatKey) and each discoverable in
// the palette. These pin the menu order, the shortcut labels, and palette discoverability. The underlying
// Monaco/LSP action is intentionally NOT exercised: no language server is bundled in the harness, and the
// regression being guarded is the missing menu affordance + command wiring, not LSP navigation itself.
...
test("editor right-click menu lists the three code-intelligence actions above the clipboard group, each with its shortcut", async ({
  page,
}) => {
...
test("the three code-intelligence commands are discoverable in the palette with their category and shortcut", async ({
  page,
}) => {
  await focusEditor(page);
