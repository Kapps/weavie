...
// session must show ITS OWN changed files / merge-base diff — never the other PR's. Each HostSession publishes
// review state on its own bus, so selection only chooses which already-scoped state is rendered.
