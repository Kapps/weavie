  ...
  weavie: [
    async ({ page, fakeScript, preNavigate, prScenario, notionDoc }, use, testInfo) => {
      ...
      try {
        ...
        // A throw here happens during setup, before the test body (or `use(host)`) ever runs. Confirmed by
        // reproduction: `testInfo.status` is NOT yet populated with the failure at this point — Playwright
        // only records a test's outcome once its whole fixture chain settles, so the `testInfo.status !==
        // testInfo.expectedStatus` check below (correct for a post-body failure) silently reads "passed"
        // here and skips the dump. This is why prior Windows `#splash` setup timeouts landed with none of
        // weavie-host.log/console-errors.txt/viewport-layout.json attached despite this exact code path
        // existing to capture them. A setup-phase failure is unconditionally a failure — dump directly
        // instead of gating on testInfo.status.
        // Flaked (Windows CI only) 2026-07-25 23:24 UTC:
        // https://github.com/Kapps/weavie/actions/runs/30178866810/job/89732005616 — diff-review.spec.ts's
        // "undoing a keep of the second hunk..." test hit this same setup-phase #splash timeout (see
        // docs/specs/e2e-flake-analysis.md's 2026-07-25 entry for the full forensics). That occurrence is
        // what surfaced this diagnostic-capture bug; the underlying Windows boot stall itself is still open,
        // waiting on the next occurrence to carry the now-fixed datum.
