...
// A window-level wheel listener — passive or not — takes the page off WebKit's async-scrolling path, so
// every surface scrolls on the main thread. The autoscroll's listeners must exist only while it is active.
test("the autoscroll registers no window wheel listener unless it is running", async ({ page }) => {
  const session = mockSession("autoscroll-listeners", "autoscroll-listeners", "codex");
  await page.addInitScript(() => {
    window.__WEAVIE_SHELL__ = {
      platform: "linux",
      titleBar: "linux",
      workspaceLabel: "autoscroll-listeners",
      recents: [],
      buildNumber: "test",
    };
    const counts = { wheel: 0 };
    (window as unknown as { __wheelListeners: () => number }).__wheelListeners = () => counts.wheel;
    const add = window.addEventListener.bind(window);
    const remove = window.removeEventListener.bind(window);
    // Listeners here are torn down by AbortController, which never calls removeEventListener.
    ...
    window.removeEventListener = (type: string, listener: never, options?: never) => {
