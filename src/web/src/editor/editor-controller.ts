...
export function createEditorController(deps: EditorControllerDeps): EditorController {
  ...
  const handleMessage = (message: WebBoundMessage): boolean => {
    switch (message.type) {
      ...
      case "turn-reset":
        // A turn boundary that clears the set (Keep-all) or a session switch: drop all inline markers so a fresh
        // set starts clean. The kept/reviewed state lives in Core now, nothing web-side to reset.
        inlineDiff?.clearAll();
        commentProse?.refresh();
        return true;
      case "fs-change": {
