...
describe("createNavHistory", () => {
  ...
  it("steps back and forward through recorded jumps", async () => {
    nav.record(location({ kind: "file", path: "/a.ts", line: 1 }));
    expect(nav.canBack()).toBe(false);
    nav.record(location({ kind: "file", path: "/b.ts", line: 1 }));
    expect(nav.canBack()).toBe(true);
    nav.record(location({ kind: "file", path: "/c.ts", line: 1 }));

    expect(nav.back()).toBe(true);
    expect(nav.canForward()).toBe(true);
    expect(visited.at(-1)).toEqual(location({ kind: "file", path: "/b.ts", line: 1 }));
    await settle();
    expect(nav.back()).toBe(true);
    expect(visited.at(-1)).toEqual(location({ kind: "file", path: "/a.ts", line: 1 }));
    await settle();
  ...
  it("coalesces small same-file moves into the current entry", () => {
    nav.record(location({ kind: "file", path: "/a.ts", line: 5 }));
    nav.record(location({ kind: "file", path: "/a.ts", line: 8 })); // < JUMP_LINES → updates in place, no new entry
    expect(nav.back()).toBe(false); // still a single entry

    nav.record(location({ kind: "file", path: "/a.ts", line: 50 })); // ≥ JUMP_LINES → a fresh jump
    expect(nav.back()).toBe(true);
    expect(visited.at(-1)).toEqual(location({ kind: "file", path: "/a.ts", line: 8 })); // returns to the latest small-move position
  });
  ...
  it("drops forward history when a fresh jump follows a back step", async () => {
    ...
    nav.record(location({ kind: "file", path: "/d.ts", line: 1 })); // new jump from b truncates c
  ...
  it("suppresses a settle-record fired mid-swap, so a step never truncates the far side", async () => {
    ...
    nav.record(location({ kind: "file", path: "/a.ts", line: 1 }));
    nav.record(location({ kind: "file", path: "/b.ts", line: 1 })); // [a, b], at b

    expect(nav.back()).toBe(true); // step to a; the swap is in flight
    nav.record(location({ kind: "file", path: "/b.ts", line: 1 })); // mid-swap settle still reports b — must be ignored
    releaseSwap?.();
    await settle();
  ...
  it("inserts the scrolled-to position when leaving a file the cursor has scrolled off", async () => {
    ...
    nav.record(location({ kind: "file", path: "/a.ts", line: 10 }));
    nav.record(location({ kind: "file", path: "/a.ts", line: 100 })); // viewport centre recorded on leaving a.ts
    nav.record(location({ kind: "file", path: "/b.ts", line: 5 }));

    expect(nav.back()).toBe(true);
    expect(visited.at(-1)).toEqual(location({ kind: "file", path: "/a.ts", line: 100 }));
    await settle();
    expect(nav.back()).toBe(true);
    expect(visited.at(-1)).toEqual(location({ kind: "file", path: "/a.ts", line: 10 }));
  });

  it("caps the history, dropping the oldest entries", () => {
    for (let i = 0; i < 60; i++) {
      nav.record(location({ kind: "file", path: `/f${i}.ts`, line: 1 }));
    }
    let steps = 0;
    while (nav.back()) {
      steps += 1;
    ...
    expect(visited.at(-1)).toEqual(location({ kind: "file", path: "/f10.ts", line: 1 })); // /f0../f9 fell off the front
  });
});
...
describe("surface navigation", () => {
  it("keeps the same file and line in review and file mode as distinct destinations", async () => {
    ...
    nav.record(origin);
    nav.record(location({ kind: "file", path: "/a.ts", line: 8 }));
    expect(nav.back()).toBe(true);
    expect(visited.at(-1)).toEqual(origin);
    await settle();
    expect(nav.forward()).toBe(true);
    expect(visited.at(-1)?.tab.kind).toBe("file");
  });

  it("explicit nearby jumps preserve the origin despite cursor coalescing", () => {
    nav.record(location({ kind: "review", path: "/a.ts", line: 8 }));
    nav.push(location({ kind: "review", path: "/a.ts", line: 9 }));
    expect(nav.back()).toBe(true);
    expect(visited.at(-1)?.view.text?.line).toBe(8);
  });
  ...
  it("failed restoration retains the actual current location and notifies after rollback", async () => {
    ...
    history.record(location({ kind: "review", path: "/a.ts", line: 8 }));
    history.record(location({ kind: "file", path: "/b.ts", line: 1 }));
...
it("a completed replacement traversal releases recording while an obsolete read remains unresolved", async () => {
  ...
  history.record(location({ kind: "review", path: "/a.ts", line: 8 }));
  history.record(location({ kind: "file", path: "/b.ts", line: 1 }));
  history.record(location({ kind: "file", path: "/c.ts", line: 1 }));
  history.back();
  history.back();
  await settle();
  expect(history.canRecord()).toBe(true);
  history.record(location({ kind: "file", path: "/d.ts", line: 1 }));
  release();
  await settle();
  expect(history.canForward()).toBe(false);
  expect(history.back()).toBe(true);
  expect(visited.at(-1)?.view.text?.path).toBe("/a.ts");
});
