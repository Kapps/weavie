...
export interface ReviseMarks {
  ...
  attach(connection: TextEditorConnection): () => void;
  dispose(): void;
}
...
export function createReviseMarks(): ReviseMarks {
  ...
  const attach = (connection: TextEditorConnection): (() => void) => {
    const { session, editor, model } = connection;
    ...
    const view = { connection, entries: () => rendered, render, dispose: teardown };
    views.add(view);
    render();
    return () => {
      views.delete(view);
  ...
  return {
    ...
    set(session: ClientSession, regions: ReviseRegion[]): void {
      ...
      for (const view of views) {
        if (view.connection.session === session) view.render();
      }
    },
    verify(session: ClientSession, id: number): string | null {
      ...
      const view = [...views].find(
        (candidate) =>
          candidate.connection.session === session &&
          normalizePath(sessionUriHostPath(candidate.connection.model.uri)) ===
            normalizePath(region.path),
      );
      const entry = view?.entries().find((candidate) => candidate.region.id === id);
      if (entry === undefined || view === undefined) return null;
      const model = view.connection.model;
...
export const sharedReviseMarks = createReviseMarks();
