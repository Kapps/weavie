...
// `lsp-exit`. Because there is no socket of its own, LSP inherits whatever transport the active backend has
// (in-process, WebSocket, or a future TLS-proxied one) — which is what lets it reach remote sessions.
...
import { activeBackendOffline, onHostMessage, postToHost } from "../bridge";
...
// A MessageWriter that posts each JSON-RPC message as an `lsp-data`. While the backend is offline it rejects
// rather than buffering into the bridge's unbounded outbox — a loud failure the client's supervised reconnect
// handles, with the reconnecting banner already on screen (no silent fallback, no unbounded growth).
...
/**
 * Opens an LSP channel: registers the demux, asks the host to spawn `server` on `slot`, and returns the
 * reader/writer to hand `monaco-languageclient`. `onExit` fires when the host reports the server ended or never
 * started (carrying its reason, e.g. "no server on PATH"). `dispose` stops the server and unregisters.
 */
export function openLspChannel(
  ...
  const writer = new BridgeMessageWriter(slot, channel);
  readers.set(channel, reader);
  exitHandlers.set(channel, onExit);
  postToHost({ type: "lsp-start", slot, server, channel });
  return {
    reader,
    writer,
    dispose: () => {
      ...
      postToHost({ type: "lsp-stop", slot, channel });
    },
  };
}
