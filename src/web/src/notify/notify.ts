...
type NotifySink = (level: Toast["level"], message: string, key?: string) => void;
 ...
 * lingering "Lost connection" error) rather than stacking a second row.
 */
export function notify(level: Toast["level"], message: string, key?: string): void {
  sink?.(level, message, key);
}
