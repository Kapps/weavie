...
public sealed class WorkspaceInvalidationWatcherTests : IDisposable {
	private readonly TempDirectory _dir = new("weavie-watch");
	...
	private bool HasChange(string fileName) {
		string path = _dir.Combine(fileName);
	...
	public async Task ReportsChange() {
		...
		await File.WriteAllTextAsync(_dir.Combine("a.ts"), "export const x = 1;\n");
	...
	public async Task IgnoreRuleChangeReEnumeratesTheWorkspace() {
		string path = _dir.Combine(".gitignore");
		Track(".gitignore");
	...
	public async Task FileCreationReEnumeratesTheWorkspace() {
		...
		Track("created.ts");
		await File.WriteAllTextAsync(_dir.Combine("created.ts"), "export {};\n");
	...
	public async Task ReportsEveryFileKind() {
		Track("notes.md");
		await using var watcher = await NewWatcherAsync();
		await File.WriteAllTextAsync(_dir.Combine("notes.md"), "hello\n");
	...
	public async Task NonRepositoryTracksNewFileFromFlatWatchEvent() {
		...
		await File.WriteAllTextAsync(_dir.Combine("new.ts"), "export {};\n");
	...
	public async Task DoesNotWatchDirectoriesAbsentFromInventory() {
		string nested = _dir.CreateDirectory("node_modules", "pkg");
		Track("real.ts");
		await using var watcher = await NewWatcherAsync();
		await File.WriteAllTextAsync(Path.Combine(nested, "dep.ts"), "export const z = 3;\n");
		await File.WriteAllTextAsync(_dir.Combine("real.ts"), "export const r = 4;\n");
	...
	public async Task ReportsDeletion() {
		string path = _dir.Combine("gone.ts");
		Track("gone.ts");
	...
	public async Task InstallsOnlyFlatInventoryDirectories() {
		string source = _dir.CreateDirectory("src", "feature");
		_dir.CreateDirectory(".git", "objects", "00");
		...
		Assert.Equal(
			new[] { _dir.Path, _dir.Combine("src"), source }.Order(),
			paths.Order());
	...
	public async Task DirectoryCreationRefreshesInventoryAndAddsFlatWatch() {
		...
		string source = _dir.CreateDirectory("src");
	...
	public async Task NonRepositoryWatchesFilesCreatedInNewDirectory() {
		...
		string nested = _dir.CreateDirectory("new-directory");
	...
	public async Task NonRepositoryDoesNotTrackIgnoredDirectoryCreatedAfterStart() {
		...
		string ignored = _dir.CreateDirectory("node_modules");
		await Task.Delay(100);
		await File.WriteAllTextAsync(Path.Combine(ignored, "ignored.ts"), "export {};\n");
		await File.WriteAllTextAsync(_dir.Combine("visible.ts"), "export {};\n");
	...
	public async Task GitTrackedDirectoryRenameReportsEveryDescendant() {
		...
		TempGitRepo.Run(_dir.Path, "add", "src/file.ts");
		...
		string renamed = _dir.Combine("renamed");
	...
	public async Task StopFlushesPendingBatch() {
		var watcher = new WorkspaceInvalidationWatcher(
			new WorkspaceInventory(_dir.Path, _ => Task.FromResult<IReadOnlyList<string>?>([])),
			batch => {
				foreach (var change in batch) {
					_changes.Add(change);
				}
