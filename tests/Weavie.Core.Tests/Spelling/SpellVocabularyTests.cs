...
public sealed class SpellVocabularyTests {
	...
	public void MixedEnglishAcceptsRegionalAndTechnicalWords(string text) {
		Assert.Empty(SpellChecker.Check(SpellVocabulary.ForLocale("en"), [new(0, 0, text, false)],
	...
	public void BroaderVocabularyStillRejectsTypos(string word) {
		Assert.Equal([new Misspelling(0, 0, word)], SpellChecker.Check(SpellVocabulary.ForLocale("en"),
	...
	public void AcronymPluralsPreserveTypoOffsetsInsideIdentifiers() {
		Assert.Equal([new Misspelling(4, 10, "Mispelled"), new Misspelling(4, 30, "ZZTYPOOs")],
			SpellChecker.Check(SpellVocabulary.ForLocale("en"), [new(4, 2, "😀 JSONsMispelled APIsCount ZZTYPOOs", false)],
				new HashSet<string>(), new HashSet<string>(), CancellationToken.None));
	}

	[Fact]
	public void SuggestionsIncludeTechnicalCorrectionsAndDeduplicateRegionalResults() {
		string[] suggestions = [.. SpellVocabulary.ForLocale("en").Suggest("middlewre")];
		Assert.Contains("middleware", suggestions);
		Assert.Equal(suggestions.Distinct(StringComparer.Ordinal), suggestions);
		Assert.Contains("misspelled", SpellVocabulary.ForLocale("en").Suggest("mispelled"));
	}
}
