// Measures the frame rate the engine Weavie.Linux ships (GTK 4 + webkitgtk-6.0) reaches on this machine.
// Run with: dotnet run tools/refresh-rate.cs [--60fps-pref] [--display-sync <libweavie-display-sync.so>]
//
// Bare, it measures WebKit's floor: without the display-sync library WebKit's DRM vblank monitor does not
// construct, and every page is paced by its hardcoded 60fps timer (a p50 of exactly 16.00ms is that timer).
// Pass --display-sync with the library the Linux host builds to measure what the host actually renders at.
// GSK_RENDERER (gl/vulkan/cairo) selects how GTK 4 draws and is worth sweeping.
...
internal partial class WebKit6Fps {
	...
	private const string DisplaySync = "libweavie-display-sync.so";
	...
	private static partial uint g_list_model_get_n_items(IntPtr model);

	[LibraryImport("libgio-2.0.so.0")]
	private static partial IntPtr g_list_model_get_item(IntPtr model, uint position);
	...
	private static void Main(string[] args) {
		...
		int syncArgument = Array.IndexOf(args, "--display-sync");
		string? displaySync = syncArgument >= 0 ? args[syncArgument + 1] : null;
		// Before GTK, exactly as the host loads it: the library only wins symbol resolution while libdrm is unloaded.
		if (displaySync is not null) {
			IntPtr handle = dlopen(Path.GetFullPath(displaySync), RtldNow | RtldGlobal);
			if (handle == IntPtr.Zero) {
				throw new InvalidOperationException($"Could not load {displaySync}.");
			}

			NativeLibrary.SetDllImportResolver(
				typeof(WebKit6Fps).Assembly, (name, _, _) => name == DisplaySync ? handle : IntPtr.Zero);
		}
		...
		Console.WriteLine($"webkitgtk-6.0 (GTK4)   PreferPageRenderingUpdatesNear60FPS:"
			...
			+ $"   display-sync: {displaySync ?? "(not loaded)"}");
	...
	private static void RegisterMonitors() {
		IntPtr monitors = gdk_display_get_monitors(gdk_display_get_default());
		for (uint index = 0; index < g_list_model_get_n_items(monitors); index++) {
			IntPtr monitor = g_list_model_get_item(monitors, index);
			try {
				Console.WriteLine($"monitor {index}: {gdk_monitor_get_width_mm(monitor)}x{gdk_monitor_get_height_mm(monitor)}mm"
					+ $" @ {gdk_monitor_get_refresh_rate(monitor) / 1000.0:0.###}Hz");
				weavie_display_sync_add_monitor(
					gdk_monitor_get_width_mm(monitor),
					gdk_monitor_get_height_mm(monitor),
					(uint)gdk_monitor_get_refresh_rate(monitor));
			} finally {
